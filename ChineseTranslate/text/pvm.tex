\section{Polkadot Virtual Machine (Polkadot 虚拟机)}\label{sec:virtualmachine}

%TODO: #446 First 64KB of memory is always inaccessible.

\subsection{Basic Definition (基本定义)}
...
我们声明通用的 \textsc{pvm} 函数 $\Psi$。我们假设一个单步调用函数定义 $\Psi_1$，并将完整的 \textsc{pvm} 递归地定义为一系列此类突变的序列，直到单步突变导致停机条件为止。我们额外定义了函数 $\deblob$，它从 \textsc{pvm} 程序 blob 中提取指令数据、操作码位掩码和动态跳转表，验证其结构，并验证给定的 $\imath$ 是否是程序中有效的指令计数器位置：
\begin{align}
  ...
  \deblob&\colon\abracegrouptwo{
    ...
    \\\\[0.2pt]
    &U\text{ 和 }T\text{ 在第~\ref{sec:basicblocks}~节中描述，\emph{skip} 在公式~\ref{eq:skip}~中描述}
  }
\end{align}

\textsc{pvm} 退出原因 $\varepsilon \in \set{\halt, \panic, \oog} \cup \set{\fault, \host} \times \pvmreg$ 可以是常规停机 $\halt$、恐慌 $\panic$ 或 gas 不足 $\oog$，或者是一个 host-call $\host$（其中关联了 host-call 标识符），或页面故障 $\fault$（此时关联了 \textsc{ram} 中的地址）。

在最终停机的情况下，无论是通过恐慌还是成功，返回的指令计数器为零。在所有其他情况下，指令计数器的返回值索引的是\emph{导致退出发生的那条指令}，而机器状态表示该指令之前的状态，从而确保\emph{事实上}的一致性。为了在这些退出情况之后继续执行，必须调整某些环境因素；对于页面故障，必须更改 \textsc{ram}；对于 gas 下溢，必须提供更多 gas；对于 host-call，必须递增指令计数器并执行相关的 host-call 状态转换。

\subsection{Instructions, Opcodes and Skip-distance (指令、操作码与跳过距离)}\label{sec:programdecoding}

\textsc{pvm} 程序 blob $\pvm¬blob$ 被拆分为一系列八位字节，组成\emph{指令数据} $\mathbf{c}$ 和\emph{操作码位掩码} $\mathbf{k}$ 以及\emph{动态跳转表} $\mathbf{j}$。前两者构成一个指令序列，进而构成一个\emph{基本块序列}，它本身是一个在\emph{块终止}指令之后的指令索引序列。

后者，即动态跳转表，是指令数据 blob 中的索引序列，在进行动态计算的跳转时对其进行索引。它被编码为一系列自然数（即非负整数），每个数字使用相同的八位字节长度编码。这个长度，即上面的项 $z$，在其之前被编码。

\textsc{pvm} 以八位字节为单位计算指令（而不是以指令为单位），因此有必要定义哪些八位字节代表一条指令的开始，\ie 操作码八位字节，以及哪些不是。这就是 $\mathbf{k}$（指令操作码位掩码）的用途。我们断言位掩码的长度等于指令 blob 的长度。

\newcommand{\Fskip}{\text{skip}}

我们定义跳过函数 $\Fskip$，它给出从当前指令操作码索引（在 $\mathbf{c}$ 中，进而在 $\mathbf{k}$ 中）到下一条指令操作码之间的八位字节数减一：
...
跳过函数在 $\mathbf{k}$ 之后附加一串置位的位序列，以确保最后一条指令 $\Fskip(\len{\mathbf{c}} - 1)$ 的结果是良定义的。

给定某个指令索引 $i$，其操作码可以方便地表示为 $\mathbf{c}_i$，向前移动到下一条指令的八位字节距离为 $1 + \Fskip(i)$。然而，每条指令的``长度''（定义为从操作码开始的连续八位字节数，需要这些八位字节来完整定义指令的语义）是隐含的，但限制为最多 16。

我们定义 $\instructions$ 等同于指令 $\mathbf{c}$，但附加了不定长的零序列，以确保不可能出现越界访问。这实际上定义了最后一条指令中原本未定义的任何参数，并确保如果程序计数器超出程序代码范围，将发生陷阱。形式化地：
\begin{equation}\label{eq:instructions}
  \instructions \equiv \mathbf{c} \concat \sq{0, 0, \dots}
\end{equation}

\subsection{Basic Blocks and Termination Instructions (基本块与终止指令)}\label{sec:basicblocks}

以下操作码的指令被视为基本块终止指令；除了 $\token{trap}$ 和 $\token{fallthrough}$ 之外，它们对应于可能将指令计数器定义为其先前值加上指令跳过量以外的值的指令：
\begin{itemize}
  \item Trap 和 fallthrough：$\token{trap}$
  , $\token{fallthrough}$
  \item 跳转：$\token{jump}$
  , $\token{jump\_ind}$
  \item 加载并跳转：$\token{load\_imm\_jump}$
  , $\token{load\_imm\_jump\_ind}$
  \item 分支：$\token{branch\_eq}$
  , $\token{branch\_ne}$
  , $\token{branch\_ge\_u}$
  , $\token{branch\_ge\_s}$
  , $\token{branch\_lt\_u}$
  ...
  \item 立即数分支：$\token{branch\_lt\_u\_imm}$
...
我们将此集合表示为操作码索引而非名称，记为 $T$，它是所有有效操作码索引 $U$ 的子集。我们定义表示基本块开始的指令操作码索引为 $\basicblocks$：
\begin{equation}
...
为方便起见，我们定义函数 $\startofbasicblock$，它表示对于该基本块内的任何 $\imath$，该基本块的起始位置。形式化地：
...
\subsection{Single-Step State Transition (单步状态转换)}\label{sec:singlestep}

我们现在必须定义单步 \textsc{pvm} 状态转换函数 $\Psi_1$：
\begin{equation}
...
在执行的第一步，以及每次执行进入新的基本块或跳回当前基本块的开头时，机器的 gas 计数器根据目标基本块的 gas 成本函数 $\gascostforblock$~(\ref{eq:gascostforblock}) 进行更新。除非整个基本块的 gas 成本已预先扣除，否则不允许在基本块内执行任何指令。如果剩余 gas 不足以覆盖全部 gas 成本，执行将被中断，gas 计数器保持不变。形式化地：
...
在执行指令的过程中，可能需要访问 \textsc{ram}。当需要访问低于 $2^{16}$ 的 \textsc{ram} 索引时，无论该值是否（不）可访问，机器都会立即 panic，不对其状态做进一步更改。否则，如果给定的 \textsc{ram} 索引不可访问，则机器状态保持不变，退出原因是一个故障，关联的是需要读取的最低不可访问\emph{页面地址}。类似地，当必须修改 \textsc{ram} 但无法进行可变访问时，机器状态不变，退出原因是一个故障，关联的是需要写入的最低不可访问页面地址。

形式化地，令 $\mathbf{r}$ 和 $\mathbf{w}$ 分别为计算 $\Psi_1$ 结果时需要对 $\mem$ 进行下标访问以进行检查和修改的索引集合。我们定义内存访问异常执行状态 $\varepsilon^\mu$ 如下：
\begin{equation}
  \begin{aligned}
...
我们还必须根据执行的指令调整状态，在必要时强制在下一步收取 gas 费用：
\begin{equation}
...
我们将 $\varepsilon$ 与常规执行中各项机器状态的后验值（以撇号标记）一起定义，如下表所示。在为指令转换机器状态时，通常满足若干条件，指令本质上由其对这些规则的例外来定义。具体而言，机器不停机，指令计数器递增一，\textsc{ram} 和寄存器保持不变。形式化地：
\begin{equation}
  \varepsilon = \continue,\quad \imath' = \imath + 1 + \Fskip(\imath),\quad \registers' = \registers,\quad\mem' = \mem \text{ except as indicated }
\end{equation}

我们定义各种八位字节宽度的有符号/无符号转换：
\begin{align}
  \label{eq:signedfunc}
...
立即数参数以小端序格式编码，最高有效位为符号位。它们可以通过省略较高有效位的八位字节来紧凑编码。如果值的 \textsc{msb} 为零，则假定被省略的八位字节为零，否则为 255。这允许对正编码值和负编码值进行紧凑表示。因此，我们定义在 $n$ 个八位字节输入上操作的符号扩展函数为 $\fnsext{n}$：
...
任何由无条件静态跳转引起的程序计数器更改必须指向基本块的起始位置，否则将发生 panic。形式化地：
\begin{equation}
...
类似地，条件跳转当且仅当两个分支都指向基本块的起始位置时才是有效的（无论分支是否被采纳），否则将发生 panic：
\begin{equation}
...
下一条指令由动态计算的跳转必须使用可以索引到跳转表 $\mathbf{j}$ 的地址。由于工具链的一个特性\footnote{流行的代码生成后端 \textsc{llvm} 在其代码生成中要求并假设动态计算的跳转目标始终具有特定的内存对齐。由于我们目前的工具链依赖于此，我们必须接受其假设。}，我们将指令所需的动态地址定义为跳转表索引加一，然后乘以我们的跳转对齐因子 $\Cpvmdynaddralign = 2$。

与程序计数器的其他非常规更改一样，目标代码索引必须是基本块的起始位置，否则将发生 panic。形式化地：
...
\subsection{Instruction Tables (指令表)}\label{sec:instructiontables}
...
\subsubsection{Instructions without Arguments (无参数指令)}
...
\subsubsection{Instructions with Arguments of One Immediate (单立即数参数指令)}
\begin{equation}
\begin{aligned}
...
\subsubsection{Instructions with Arguments of One Register and One Extended Width Immediate (单寄存器与单扩展宽度立即数参数指令)}
\begin{equation}
...
\subsubsection{Instructions with Arguments of Two Immediates (双立即数参数指令)}
\begin{equation}
\begin{aligned}
...
\subsubsection{Instructions with Arguments of One Offset (单偏移参数指令)}
\begin{equation}
\begin{aligned}
  \using l_X = \min(4, \ell) \,,\quad
...
\subsubsection{Instructions with Arguments of One Register \& One Immediate (单寄存器与单立即数参数指令)}
\begin{equation}
\begin{aligned}
    \using r_A &= \min(12, \instructions_{\imath+1} \bmod 16) \,,\quad&
...
\subsubsection{Instructions with Arguments of One Register \& Two Immediates (单寄存器与双立即数参数指令)}
\begin{equation}
\begin{aligned}
    \using r_A &= \min(12, \instructions_{\imath+1} \bmod 16) \,,\quad&
    \reg_A &\equiv \reg_{r_A} \,,\quad
...
\subsubsection{Instructions with Arguments of One Register, One Immediate and One Offset (单寄存器、单立即数与单偏移参数指令)}
\begin{equation}
  \begin{aligned}
      \using r_A &= \min(12, \instructions_{\imath+1} \bmod 16) \,,\quad&
      \reg_A &\equiv \reg_{r_A} \,,\quad
...
\subsubsection{Instructions with Arguments of Two Registers (双寄存器参数指令)}
\begin{equation}
\begin{aligned}
...
\subsubsection{Instructions with Arguments of Two Registers \& One Immediate (双寄存器与单立即数参数指令)}
\begin{equation}
\begin{aligned}
  \using r_A &= \min(12, (\instructions_{\imath+1}) \bmod 16) \,,\quad&
  \reg_A &\equiv \reg_{r_A} \,,\quad
...
\subsubsection{Instructions with Arguments of Two Registers \& One Offset (双寄存器与单偏移参数指令)}
\begin{equation}
  \begin{aligned}
    \using r_A &= \min(12, (\instructions_{\imath+1}) \bmod 16) \,,\quad&
    \reg_A &\equiv \reg_{r_A} \,,\quad
...
\subsubsection{Instruction with Arguments of Two Registers and Two Immediates (双寄存器与双立即数参数指令)}
...
\subsubsection{Instructions with Arguments of Three Registers (三寄存器参数指令)}
\begin{equation}
\begin{aligned}
  \using r_A &= \min(12, (\instructions_{\imath+1}) \bmod 16) \,,\quad&
  \reg_A &\equiv \reg_{r_A} \,,\quad
...
注意，两个有符号取模运算具有特殊的定义，它们对绝对值进行取模运算，但保留分子的符号。形式化地：
\begin{equation}
...
\subsection{Host Call Definition (Host Call 定义)}

\textsc{pvm} 调用的扩展版本能够在 host-call 停机条件下推进内部\emph{host-call}状态机，定义为 $\Psi_H$：
\begin{align}
...
与 $\Phi$ 类似，退出时指令计数器引用的是\emph{导致退出的指令}，机器状态是该指令之前的状态。如果使用该指令计数器和代码再次调用机器，则导致退出的同一条指令将在正确的（先前的）机器状态上执行。

使用 $\Phi_H$，host-call（\ie \token{ecalli} 指令）实际上由作为参数提供的状态修改函数内部处理，防止了结果为 host-call 故障的可能性。注意，在成功的 host-call 转换情况下，我们必须在该指令的新后验状态旁边显式提供新的指令计数器值 $\imath''$。

\subsection{Standard Program Initialization (标准程序初始化)}\label{sec:standardprograminit}

在主文档中使用 \textsc{pvm} 的三个实例中运行的软件程序具有编译器和链接器输出的非常典型的设置模式。这意味着 \textsc{ram} 具有程序特定的只读数据、读写（堆）数据和栈的段。与此相关，非常符合我们使用模式的是一个额外的只读段，通过它可以传递调用特定的数据（\ie 参数）。因此有必要在单个初始化函数中正确定义这些内容。这些段被量化为\emph{主区域}，并且在段之间始终留有一个未分配的主区域，以减少意外越界。段被填充零到最近的 \textsc{pvm} 内存页边界。

因此，我们定义标准 JAM 程序 blob 格式 $\jam¬blob$，它不仅包含原始的 \textsc{pvm} 程序 blob $\pvm¬blob$，还包含程序启动时 \textsc{ram} 状态的信息。给定 JAM 程序 blob $\jam¬blob$ 和参数数据 $\mathbf{a}$，我们可以通过调用标准初始化函数 $Y(\jam¬blob, \mathbf{a})$ 来解码 \textsc{pvm} 程序 blob $\pvm¬blob$、寄存器 $\registers$ 和 \textsc{ram} $\mem$：
\begin{equation}
Y\colon\abracegroup{
  \tuple{\blob, \blob[:\Cpvminitinputsize]} &\to \tuple{\blob, \regs, \ram}? \\\\
  \tup{\jam¬blob, \mathbf{a}} &\mapsto \begin{cases}
    \tup{\pvm¬blob, \registers, \mem} &\when \exists! \tup{\pvm¬blob, \mathbf{o}, \mathbf{w}, z, s} \text{ 满足公式~\ref{eq:conditions}}\\\\
    \none &\otherwise
  \end{cases}
}
\end{equation}
...
\end{align}
因此，如果上述条件无法以唯一值满足，则结果为 $\none$，否则结果为 $\pvm¬blob$（如上所述）、$\mem$ 和 $\registers$ 的元组，使得：
...
\subsection{Argument Invocation Definition (参数调用定义)}

\textsc{pvm} 使用的三个实例都期望能够传入参数数据并接收一些返回数据。因此我们定义通用的 \textsc{pvm} 程序参数调用函数 $\Psi_M$：
\begin{equation}
...
注意，第一个元组项是操作消耗的 gas 量，但永远不会大于为操作提供的 gas 量。

\subsection{Gas Cost Model (Gas 成本模型)}\label{sec:gascostmodel}
...
\textsc{pvm} 的 gas 成本模型是现代 CPU 微架构的简化模型，深受生产级编译器用于预测给定代码片段执行时间的方法启发。对于程序中的每个基本块，该模型模拟其执行流程并计算执行它所需的虚拟 CPU 周期数。

gas 模拟状态，属于集合 $\gas¬sim¬ty$，包含当前指令计数器 $\imath$、周期计数器 $\ctr¬cycles$、当前周期中剩余的解码槽位数 $\remaining¬decodeslots$、当前周期中仍允许开始执行的最大指令数 $\remaining¬startpercycle$、剩余可用执行单元 $\remaining¬execunits$ 和重排序缓冲区 $\rob$。形式化地：
...
重排序缓冲区条目，属于集合 $\rob¬entry¬ty$，包含其当前状态 $\rob¬state$、剩余周期数 $\rob¬cyclesleft$、被视为其依赖项的重排序缓冲区索引集合 $\rob¬deps$、被破坏的寄存器集合 $\rob¬regs$ 和使用的执行单元 $\rob¬execunits$。形式化地：
...
集合 $\execunits¬ty$ 的元组将每种执行单元类型（${\ALU, \LOAD, \STORE, \MUL, \DIV}$）映射为一个数字：
...
为方便起见，我们将两个 $\execunits¬ty$ 集合元组的加法和减法定义为按成员进行的同类型运算；形式化地：
...
类似地，两个 $\execunits¬ty$ 集合元组之间的比较当且仅当所选比较对所有对应成员都成立时才为真：
...
我们定义函数 $\inst¬srcregs(\mathbf{c}, \mathbf{k}, \imath)$，它返回 $\imath$ 处指令读取的源寄存器集合，以及函数 $\inst¬dstregs(\mathbf{c}, \mathbf{k}, \imath)$，它返回 $\imath$ 处指令写入的目标寄存器集合，如 \ref{sec:instructiontables} 节中的方程所述。这与这些寄存器在运行时执行时是否实际会被该指令修改无关。在此模型中，假设 \token{ecalli} 既不读取也不写入任何寄存器。

我们还定义函数 $\cost¬cycles(\mathbf{c}, \mathbf{k}, \imath)$，它返回 $\imath$ 处指令完成执行所需的周期数；$\cost¬decodeslots(\mathbf{c}, \mathbf{k}, \imath)$，它返回解码该指令所需的解码槽位数；以及 $\cost¬execunits(\mathbf{c}, \mathbf{k}, \imath)$，它返回开始执行该指令所需的虚拟 CPU 执行单元数。这些函数简单地返回 \ref{sec:gascosttables} 节中指定的值。

从指令操作码索引 $\imath \in \basicblocks$ 开始的给定基本块的 gas 成本，由 gas 成本模型转换函数确定的虚拟 CPU 周期数定义，直到该基本块已吸收的所有指令都已退役且模拟已经收敛。形式化地：
...
状态转换函数 $\gas¬sim'$ 解码指令而不触发虚拟 CPU 流水线模拟，定义如下：
...
\token{move\_reg} 指令被特殊处理，由我们虚拟 CPU 的前端处理，不添加到重排序缓冲区：
...
\begin{equation}
  \gas¬sim^{\text{mov}}\colon \abracegrouptwo{
    ...
    \where \gas¬sim¬state' &= \gas¬sim¬state\text{ 除以下外：}\\\\
      ...
      \gas¬sim¬state'_{\rob} &= \in¬rob\text{ 除以下外：}\\\\
      &\begin{aligned}
...
\begin{equation}
  \gas¬sim^{\text{dec}}\colon \abracegrouptwo{
    ...
    \end{cases}\\\\
    \mathbf{c} &\mapsto \instructions \text{ 根据 \ref{eq:instructions}}\\\\
    ...
    \rob &= \in¬rob\text{ 除以下外： } \forall j \in \in¬rob¬len : \rob¬entry¬field{\rob}{j}{\rob¬regs} = \in¬rob¬field{j}{\rob¬regs} \setminus \rob¬regs\\\\
...
状态转换函数 $\gas¬sim''$ 开始执行下一条待处理指令，定义如下：
...
\begin{equation}
  \gas¬sim''\colon \abracegrouptwo{
    ...
    \gas¬sim¬state'_{\rob} &= \in¬rob\text{ 除以下外： } \rob¬entry¬field{\gas¬sim¬state'_{\rob}}{n}{\rob¬state} = \state¬executing
  }
\end{equation}

函数 $\find¬readytostart$ 检查重排序缓冲区中哪条指令准备好开始执行（以及是否存在这样的指令），定义如下：
...
状态转换函数 $\gas¬sim'''$ 模拟虚拟 CPU 流水线的其余部分，定义如下：
...
\begin{equation}
  \gas¬sim'''\colon \abracegrouptwo{
    ...
    \gas¬sim¬state'_{\rob} &= \in¬rob\text{ 除以下外 } \forall j \in \in¬rob¬len: \\\\
    &\begin{aligned}
...
\subsection{Gas Cost Tables (Gas 成本表)}\label{sec:gascosttables}

对于某些指令，其成本取决于目标寄存器是否与任何源寄存器重叠：
...
\begin{equation}
  \mathfrak{P}_{S}\colon \abracegrouptwo{
    ...
    \\\\[0.2pt]
    &\where (\mathbf{c}, \mathbf{k}, \imath) \mapsto (\reg_A, \reg_D) \text{ 根据 \ref{sec:programdecoding} 和 \ref{sec:instructiontables}} \\\\
  }
\end{equation}
...
分支的成本取决于其目标（跳转目标或隐式 fallthrough）是否指向等于 \token{unlikely} 或 \token{trap} 指令操作码的指令字节；形式化地：
...
\begin{equation}
  \mathfrak{b}\colon \abracegrouptwo{
    ...
    \\\\[0.2pt]
    \where &\mathbf{c} \mapsto \instructions \text{ 根据 \ref{eq:instructions}}\\\\
      &\imath_{\text{target}} \text{ 是根据 \ref{sec:instructiontables} 得出的分支目标}
  }
\end{equation}
...
在下表中，为清晰起见省略了 $\mathbf{c}$、$\mathbf{k}$ 和 $\imath$ 参数。
